    ...
    <>
      <div className="flex flex-col border border-black">
        ...
        {blocksInfo.map((el, idx) => (
          <div key={el} className={idx !== blocksInfo.length - 1  ? "border border-b-black border-b-2 h-[15%] flex items-center justify-between px-5 text-xs" : "border h-[15%] flex items-center justify-between px-5 text-xs"}>
          ...
          <div className="flex flex-col gap-2">
            ...
            <h2>{el.transactions && el.transactions.length} txns <span className="text-[9px] text-gray-400">in 12 secs</span></h2>
          </div>
        </div>
        ))}
      </div>
